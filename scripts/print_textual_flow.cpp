...
int main(int argc, char* argv[]) {
	...
	try {
		...
		options.custom_help("[-h] [-e wit_1 -e wit_2 ...] [-p proportion] [-k connectivity] [--flow] [--attestations] [--variants] [--strengths] input_db [passages]");
		options.positional_help("").show_positional_help();
		options.add_options("")
				("h,help", "print this help")
				("e,excluded", "IDs of witnesses to exclude from the textual flow diagram", cxxopts::value<vector<string>>())
				("p,proportion_extant", "minimum proportion of variation units at which a witness must be extant to be included in the textual flow diagram", cxxopts::value<float>())
